...
Thus the covariance-normalized **uncut entry seed** has physical norm

\[
\boxed{\ |u_{\rm seed,modal}|\asymp\mathcal A e^{-G_*}
=Q^{-1/2-h/2}L^{1/4}\sqrt{|T|}|\eta_\beta\chi_g|e^{-G_*}.\ }
\tag{7}
\]

It is super-polynomially small as the band grows, despite the target stress's singular physical scale.
...
At `r=±2e/λmin` the vector field points inward. Consequently `|r|≤2e/λmin≤1/4`, and

`(log z+)′=λ−d+E11+E12r=λ−dref+O(1/S)`.

Since `L/S` is bounded, `|V(L/2,0)e+|=exp(G_*+O(1))`. The upper energy estimate gives `||V(L/2,0)||≤exp(G_*+O(1))`. This proves (7), as well as its necessity up to constants for a target of size `𝒜`: no smaller-norm seed can produce that target in this ODE. Bounded frame maps convert modal and ambient norms.
...
2. Its quadratic nonzero source is in `W(1−κs)` and initially has `m=±2`. The zero-initial-data modal inverse preserves this exponent, costing only powers of `S`; `m=2` also has the extra dissipative factor `exp(−3∫d)` from (7.18).
3. Pairing this response with the primary can return to `m=±1`, but yields only `W(3/2−2κs)`. Compared with the prescribed `W1/2` pulse, the **available upper scale** has the additional factor `ε^(1−2κs)` up to finite powers of `S` (away from vanishing edges).
...
**Precise review targets:** (i) (5)–(10), especially the absolute damping input and the distinction between backward output selectivity and forward robustness; (ii) the scope boundary of the conditional storage calculation (12); (iii) same-label cancellation, support separation and the finite-stage return exponent in §5; (iv) selection/domain routing in §6. A failure of any proposed channel excludes that channel only.
