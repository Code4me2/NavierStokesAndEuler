...
Both libraries localise whole-space arguments with the same family of cutoffs:
...
`NavierStokes/R3/ComparisonCutoffs.lean` and `Euler/FiniteEnergyTruncation.lean`
instantiate this on `EuclideanSpace ℝ (Fin 3)` under the names their importers
use.  The space is an explicit argument of the definitions so that partial
applications such as `cutoff E R` elaborate without an expected type.
-/
...
private def dilation (E : Type*) [NormedAddCommGroup E] [InnerProductSpace ℝ E] (R : ℝ) :
    E →L[ℝ] E :=
  R⁻¹ • ContinuousLinearMap.id ℝ E

private theorem norm_dilation_le {R : ℝ} (hR : 0 < R) : ‖dilation E R‖ ≤ R⁻¹ := by
  apply ContinuousLinearMap.opNorm_le_bound _ (inv_nonneg.mpr hR.le)
  intro x
  change ‖R⁻¹ • x‖ ≤ R⁻¹ * ‖x‖
  rw [norm_inv_smul hR, div_eq_inv_mul]

/-- Dilating a smooth function by `R⁻¹` divides a uniform bound on its `n`th
derivative by `R ^ n`. -/
theorem iteratedFDeriv_comp_inv_smul_le {χ : E → F} (hχ : ContDiff ℝ ∞ χ) {n : ℕ} {C : ℝ}
    (hC : ∀ y, ‖iteratedFDeriv ℝ n χ y‖ ≤ C) {R : ℝ} (hR : 0 < R) (x : E) :
    ‖iteratedFDeriv ℝ n (fun y => χ (R⁻¹ • y)) x‖ ≤ C / R ^ n := by
  have hC0 : 0 ≤ C := (norm_nonneg _).trans (hC x)
  change ‖iteratedFDeriv ℝ n (χ ∘ dilation E R) x‖ ≤ _
  rw [(dilation E R).iteratedFDeriv_comp_right (contDiff_infty.1 hχ n) x le_rfl]
  calc
    ‖(iteratedFDeriv ℝ n χ (dilation E R x)).compContinuousLinearMap
        (fun _ => dilation E R)‖
        ≤ ‖iteratedFDeriv ℝ n χ (dilation E R x)‖ * ∏ _ : Fin n, ‖dilation E R‖ :=
      ContinuousMultilinearMap.norm_compContinuousLinearMap_le _ _
    _ ≤ ‖iteratedFDeriv ℝ n χ (dilation E R x)‖ * (R⁻¹) ^ n := by
      apply mul_le_mul_of_nonneg_left _ (norm_nonneg _)
      calc
        ∏ _ : Fin n, ‖dilation E R‖ ≤ ∏ _ : Fin n, R⁻¹ :=
...
private theorem exists_derivative_bound (n : ℕ) :
  ...
  have hcont : Continuous (iteratedFDeriv ℝ n (baseCutoff E)) :=
    baseCutoff_smooth.continuous_iteratedFDeriv (by simp)
...
theorem cutoff_iteratedFDeriv_le {R : ℝ} (hR : 0 < R) (n : ℕ) (x : E) :
  ...
  iteratedFDeriv_comp_inv_smul_le baseCutoff_smooth (baseCutoff_iteratedFDeriv_le n) hR x
